...
* `exporter/kube_collector.go` — In-cluster-only `kuber_*` job-resource metrics. Registered on the same registry as `ProcessCollector`, so a panic here takes the whole exporter down; its `job_name` label is `/proc`-sourced and gets the same sanitize+bound treatment. Build it with `NewKubeCollectorWithConfig` — `job_name` is the same `CI_JOB_NAME` value as `ci_job_name`, so it must be bound with the same overridden limit and count its cuts on the same counter; `NewKubeCollector` keeps the DEFAULT table and counts nothing.
...
* In `scrubEnviron`, an over-long value is replaced WHOLE by `environTruncMarker()` — never truncated to a prefix, and never fingerprinted (length only). `environ` is the only label built from arbitrary operator-unknown pairs, so length alone is a secret signal there: `isSensitivePair` only recognises token-*shaped* values (`isTokenCharset` rejects braces, quotes, colons, newlines), so a JSON service-account key or PEM body matches nothing and a prefix would publish credential material. It gets no digest because even a salted one is a within-process EQUALITY oracle on an assumed-secret value (same credential on two hosts; digest changes on rotation). The other bounded labels (`name`, `cmdline`, `ci_*`) keep prefix + fingerprint via `truncateWithFingerprint`.
* `fingerprint()` MUST stay salted with `fingerprintSalt` and that salt MUST never be exposed in a label, metric, or log. `cmdline` is raw argv and is the one bounded label with NO secret redaction on any path, so an unsalted digest would publish a verifiable commitment to the credential bytes past the cut on a `/metrics` every scraper reads. Consequence to preserve in docs: truncation is deterministic only *within* one exporter process — a restart re-rolls the salt and churns truncated series once.
