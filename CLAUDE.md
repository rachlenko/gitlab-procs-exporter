...
* That covers UTF-8 validity but not length: every label value MUST then pass `boundLabel()` (the collector method, so the cut is counted) before reaching `MustNewConstMetric`. Order is fixed — `sanitizeLabelValue()`, then redact, then bound; bounding invalid UTF-8 makes the rune walk-back meaningless. A new label needs an entry in `MaxLabelBytes` (`exporter/labelbound.go`) or it silently passes through unbounded, and limits are BYTE counts, never rune counts. `environ` is the one exception: it is a composed blob bounded by `maxEnvironVars`/`maxEnvironValueLen`/`maxEnvironBytes` in `scrubEnviron` instead.
