...
* In `scrubEnviron`, an over-long value is replaced WHOLE by `environTruncMarker()` — never truncated to a prefix, and never fingerprinted (length only; an unsalted digest of an unclassified value is an offline confirmation oracle on a public `/metrics`). `environ` is the only label built from arbitrary operator-unknown pairs, so length alone is a secret signal there: `isSensitivePair` only recognises token-*shaped* values (`isTokenCharset` rejects braces, quotes, colons, newlines), so a JSON service-account key or PEM body matches nothing and a prefix would publish credential material. Labels with a known, non-secret shape (`name`, `cmdline`, `ci_*`) keep their prefix via `truncateWithFingerprint`.
